...
public class ProviderResource2_28 extends MetadataDelegatingCrudResource<Provider> {
	...
	public Model getGETModel(Representation rep) {
		...
		if (rep instanceof FullRepresentation) {
			model.property("person", new RefProperty("#/definitions/PersonGet")).property("attributes",
			    new ArrayProperty(new RefProperty("#/definitions/ProviderAttributeGet")));
		}
		return model;
	}
	...
	public Model getCREATEModel(Representation rep) {
		ModelImpl model = ((ModelImpl) super.getCREATEModel(rep)).property("person", new StringProperty().example("uuid"))
		        .property("identifier", new StringProperty())
	...
	protected PageableResult doSearch(RequestContext context) {
		...
		String enterpriseGuid = Context.getProviderService().getEnterpriseForLoggedinUser();
		List<Provider> providers = Context.getProviderService().getProviders(query, context.getStartIndex(),
		    context.getLimit(), null, context.getIncludeAll(), enterpriseGuid);
